...
def format_human_readable(query: str, code_refs: list, afs_refs: list, 
    ...
    if code_refs:
        ...
        for ref in code_refs:
            ...
            output.append(f"  - {path} (repo: {repo}, last modified: {last_mod}, author: {author})")
        output.append("")
...
def format_json(query: str, code_refs: list, afs_refs: list, 
                summary: Optional[dict] = None) -> str:
...
@click.option("--summary", "generate_summary", is_flag=True, help="Generate human-readable summary (default)")
@click.option("--depth", type=int, default=None, help="Limit search depth for cross-references (not yet implemented)")
@click.option("--config", type=click.Path(exists=True), help="Path to config.yaml file")
def main(query: str, output_json: bool, generate_summary: bool, depth: Optional[int], config: Optional[str]):
